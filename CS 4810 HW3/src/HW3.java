...
public class HW3 {
		...
		for (int row = 0; row < height; row++) {
			for (int col = 0; col < width; col++) {
				...
				for (Sphere sphere : spheres) {
					double intersect = HW3.RayIntersectSphere(ray, sphere);
				...
				for (Plane plane : planes) {
					double intersect = HW3.RayIntersectPlane(ray, plane);
					if (intersect >= 0 && intersect < closest) {
						closest = intersect;
				...
				if (closestObject != null) {
					...
					// apply lighting
					for (Vertex sun : suns) {
						if (!HW3.isSunBlocked(sun, closestLocation,
								closestObject)) {
					...
					for (Vertex bulb : bulbs) {
						if (!HW3.isBulbBlocked(bulb, closestLocation,
								closestObject)) {
							double nDotI = closestNormal.normalize()
									.dotProduct(
											bulb.getVector()
	...
	public static boolean isSunBlocked(Vertex sun, Vector location, Object o) {
		...
		for (Sphere sphere : HW3.spheres) {
			if (sphere != o && HW3.RayIntersectSphere(rayToSun, sphere) > 0) {
				return true;
			}
		}
		for (Plane plane : HW3.planes) {
			if (plane != o && HW3.RayIntersectPlane(rayToSun, plane) > 0) {
				return true;
			}
		}
		return false;
	...
	public static boolean isBulbBlocked(Vertex bulb, Vector location, Object o) {
		...
		double tToBulb = 1;
		for (Sphere sphere : HW3.spheres) {
			double t = HW3.RayIntersectSphere(rayToBulb, sphere);
			if (sphere != o && t > 0 && t < 1) {
				return true;
			}
		}
		for (Plane plane : HW3.planes) {
			double t = HW3.RayIntersectPlane(rayToBulb, plane);
			if (plane != o && t > 0 && t < 1) {
				return true;
			}
		}
